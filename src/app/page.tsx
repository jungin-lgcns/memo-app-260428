...
export default function Home() {
  ...
  const detailMemo = useMemo(() => {
    if (!detailMemoId) return null
    return allMemos.find(m => m.id === detailMemoId) ?? null
  }, [detailMemoId, allMemos])

  useEffect(() => {
    if (detailMemoId && !detailMemo) {
      setDetailMemoId(null)
    }
  }, [detailMemoId, detailMemo])
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <MemoList
          ...
          onOpenDetailMemo={memo => setDetailMemoId(memo.id)}
